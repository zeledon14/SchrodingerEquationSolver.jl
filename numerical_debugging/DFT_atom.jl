...
let
    ...
    names = String[]
    E_pred = Float64[]  # Assuming energy values are floating-point numbers
    E_targ = Float64[]


    for (key, value) in pred_energy_dict
        push!(names, key)  # Append key to names
        push!(E_pred, value)  # Append predicted energy
        push!(E_targ, energy_target[key])  # Append target energy
    end
    ...
    table_data = [names E_targ E_pred E_diff Perc_error]
    pretty_table(table_data, header=["Name", "E NIST", "E pred", "|ΔE|" , "|ΔE|/E NIST" ])
end#let
